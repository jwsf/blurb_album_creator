...
### 9. Export to PDF
Delegate PDF conversion to the separate `blurb-to-pdf` skill — **it is the
single source of truth for what conversion supports, its known
limitations, and its internal mechanisms.** Don't duplicate that detail
here; it drifts out of sync with the actual script every time either one
changes without the other. See `.claude/skills/blurb-to-pdf/SKILL.md`.
...
**Requirements:** see `.claude/skills/blurb-to-pdf/SKILL.md` (reportlab, Pillow, and on macOS the system Apple Color Emoji font for emoji rendering specifically).
